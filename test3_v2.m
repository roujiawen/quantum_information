...
results_data_path = 'results/test3final_noxu';
...
use_xu = true;
%% I/O
test_data_path = 'logs/test_cases_500trials/verified_infea_test_cases_v1.mat';
load(test_data_path);
ntrials = size(test_cases, 2);
results_data_path = 'results/test3final_xu_intval';
...
fprintf('%d %s %s\n', k, slack, solver);
% Get basis formulation and slack formulation
[constraint_A, constraint_b] = get_spiral_form(const_type, basis, P_ABC, true);
[A, b, c, K] = get_slack_vsdp_form(constraint_A, constraint_b, slack);
% Run VSDP wrapper
[verified_lower, verified_upper, stats] = vsdp_wrapper(A, b, c, K, solver, 0, [], use_xu);
% Add data row to master table
...
load(test_data_path);
ntrials = size(test_cases, 2);
results_data_path = 'results/test3final_noxu_intval';
